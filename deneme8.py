...
def upload_mission(vehicle):
    ...
    # 4) “∞” deseni: Pole1 etrafı CCW, Pole2 etrafı CW
    # 4.a) Pole1 etrafında tam çember (CCW)
    center1 = LocationGlobalRelative(POLE1[0], POLE1[1], pattern_alt)
    for i in range(pattern_steps + 1):
        ang = 360.0 * i / pattern_steps
        dN  = pattern_radius * math.cos(math.radians(ang))
        dE  = pattern_radius * math.sin(math.radians(ang))
    ...
    # 4.b) Pole2 etrafında tam çember (CW)
    center2 = LocationGlobalRelative(POLE2[0], POLE2[1], pattern_alt)
    for i in range(pattern_steps + 1):
        ang = 360.0 * (pattern_steps - i) / pattern_steps  # ters yön
        dN  = pattern_radius * math.cos(math.radians(ang))
        dE  = pattern_radius * math.sin(math.radians(ang))
